...
- **Past the threshold**: rather than switch on Anthropic's official usage API — safe to poll only every ~60s, so a heavy subagent turn regularly burns past the threshold before the next poll catches it — cswap sends `/usage` through the account's own isolated session profile (same isolation as `--warm-on-reset`, never the active credential) and watches the real number: every 60s from the threshold up to 96%, 30s from 96-99%, 20s from 99% up. It's a local CLI command, not a real request (no model turn, `$0`, no tokens), so it isn't subject to the official endpoint's rate budget and can be polled this tightly the whole way. The switch fires the instant a fresh read reports 100% on either the 5-hour *or* 7-day window — so a weekly cap hit while the session window is nowhere close still switches right away.
