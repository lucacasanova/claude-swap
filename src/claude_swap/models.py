...
class AccountsSnapshot:
    ...
    # (``{"number", "at", "session_pct", "week_pct"}``) if some `cswap auto`
    # has it past `threshold` right now, else ``None``. Display-only, same
    # as ``pinging`` — never affects ``active_number`` or ``is_active``.
